...
\item Implemente a expressão condicional \p{corner-direita}. Isto pode ser implementado como uma extensão da \p{wall-direita}. Quando o valor da palavra \p{parede direita} mudar de verdadeiro para falso, faça uma curva curta à direita e verifique se a palavra p{parede direita} se torna verdadeira novamente.
\end{itemize}
\end{framed}
...
Infelizmente, o algoritmo pode falhar quando confrontado com um obstáculo em forma de \emph{G} (Fig.~\ref{fig.wall-direction-bug}). Depois de fazer quatro voltas à esquerda, seu título é $360^\circ$ (também ao norte, um múltiplo de $360^\circ$) e continua avançando, encontrando e seguindo a parede uma e outra vez.
...
\noindent\textbf{\p{goto nest}:} Este estado é semelhante ao estado {seguinte} em que o robô avança em direção ao ninho, virando à direita ou à esquerda conforme necessário para se mover na direção do ninho. Quando atinge o ninho, ele retorna ao estado de \p{search}.
