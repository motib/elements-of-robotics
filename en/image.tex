...
\index{image processing!resolution}
...
The final phase of image processing is to recognize objects\index{image processing!recognition}. In Sect.~\ref{s.corners}, we present two algorithms for detecting corners: by locating the intersection of two edges and by counting neighbors with similar intensities. Section~\ref{s.blob} describes how to recognize \emph{blobs}, which are areas whose pixels have similar intensities but which are not bounded by regular features such as lines and curves. Finally, Activity~\ref{act.recognize} demonstrates the recognition of an object that is defined by more than one feature, such as a door defined by two edges that are at an arbitrary distance from each other.

\section{Image enhancement}\label{s.enhance}
\index{image processing!enhancement}
...
\index{image processing!spatial filter}
...
The \emph{box filter}\index{image processing!box filter} is:
\[
\left[
\begin{array}{ccc}
1 & 1 & 1\\
...
The box filter gives equal importance to the pixel and all its neighbors, but a \emph{weighted filter}\index{image processing!weighted filter} uses different factors for different pixels. The following filter gives much more weight to the pixel itself than to its neighbors:
\[
\left[
\begin{array}{ccc}
1 & 1 & 1\\
...
\index{image processing!histogram manipulation}
...
\index{image processing!edge detection}
...
There are many digital derivative operators. A simple but effective one is the \emph{Sobel filter}\index{image processing!Sobel filter}. There are two filters, one for detecting horizontal edges (on the left) and other for detecting vertical edges (on the right):
\[
...
\index{image processing!corner detection}
